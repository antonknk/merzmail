...
greens_rss <- tidyRSS::tidyfeed("https://www.gruene-bundestag.de/presse/rss.xml")
...
# accept cookies
cookie_button <- remote_driver$findElement(using = "css selector", "button.button")
...
load_more <- function(rd) {
  ...
  load_more_button <- rd$findElement(using = "css selector", "button.button")
  # ... click it
...
# fulltext 
page %>% 
  html_element("article > div > div > section > section > div > div > div > p:not(:nth-child(1))")  %>% 
  html_text2()
